...
변경된 `.cs` 파일만 리뷰 대상으로 삼는다. untracked `.cs` 파일도 포함하여 전체 내용을 읽는다. 문서, 메타, 설정 파일은 제외. **변경된 `.cs` 파일이 없으면 "리뷰 대상 없음"을 출력하고 종료한다.**
...
**변경된 코드와 새로 생성된 코드(untracked)에 집중**하며, 기존 코드의 문제를 지적하지 않는다. 새 파일은 전체 내용이 리뷰 대상이다.
...
- 각 Agent에게 파일 경로 + 해당 파일의 diff + 체크리스트 **내용**(Read한 결과를 프롬프트에 직접 포함)을 전달하여 병렬 검토
...
**심각도 기준**:
- **치명**: 런타임 크래시, 데이터 레이스, 네트워크 비동기, Health 직접 수정, 메모리 누수 (Persistent Dispose 누락, TempJob 미해제)
- **경고**: 컨벤션 위반, Burst 누락, 성능 문제 (O(n²) 탐색, 불필요한 CompleteDependency, 매 프레임 과다 할당, Lookup ReadOnly 미적용), 누락된 의존성 선언, GameSettings 미사용
- **제안**: 더 나은 대안 존재, 가독성 개선, 사소한 네이밍 문제, 성능 미세 최적화 (Capacity 조정, 병렬화 기회)
...
- **변경/신규 코드만 리뷰**: 기존 코드의 문제를 지적하지 않는다. 단, 변경으로 인해 기존 코드와의 정합성이 깨지는 경우는 지적한다. untracked 새 파일(`.cs`)은 전체 내용을 리뷰 대상으로 삼는다.
