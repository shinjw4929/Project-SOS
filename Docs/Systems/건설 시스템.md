...
└─ [사거리 밖] BuildMoveRequestRpc → HandleBuildMoveRequestSystem
       ...
       ↓
FlowFieldSystem → FlowFieldSteeringSystem → PredictedMovementSystem
       ↓
...
| HandleBuildMoveRequestSystem.cs | SimulationSystemGroup (UpdateAfter: UnifiedTargetingSystem) | BuildMoveRequestRpc 수신 → GhostIdMap으로 Entity 변환, 소유권 검증, MovementGoal 설정(건물 가장자리에서 workRange*0.5f 바깥 오프셋, DestinationSetTime 초기화), UnitIntentState=Build, AggroTarget 초기화, PendingBuildServerData 추가, MovementWaypoints 활성화, ArrivalRadius=ArrivalUtility.GetSafeArrivalRadius(workRange, 0f) |
| BuildArrivalSystem.cs | SimulationSystemGroup (UpdateAfter: MovementArrivalSystem) | PendingBuildServerData 보유 유닛 도착 감지 (MovementWaypoints 비활성화 OR 사거리 내+저속<0.1f) → arrivalDist = GetInteractionArrivalDistance(StructureRadius, workRange) + CellSize×0.7f (FlowField 셀 오차 보상) → 사거리 내: 자원 확인/차감 → 건물 생성 → 정리. 사거리 밖 재시도(max 3): BuildSiteCenter 직접 목표로 변경 + 이동 재개, 3회 초과 시 포기(Intent=Idle) |
...
| 파일 | 역할 |
| --- | --- |
| ArrivalUtility.cs | 도착 거리 공통 유틸리티 (Burst 호환). GetSafeArrivalRadius(Dead Zone 없는 ArrivalRadius 계산), GetInteractionArrivalDistance(상호작용 도착 거리 = targetRadius + workRange), IsWithinInteractionRangeXZ(XZ 2D 거리 판정). HandleBuildMoveRequestSystem, BuildArrivalSystem에서 공유 |
...
| 파일 | 역할 |
| --- | --- |
| PendingBuildServerData.cs | 이동 후 건설용 대기 데이터: `StructureIndex`, `GridPosition`, `BuildSiteCenter`, `StructureRadius`, `OwnerNetworkId`, `SourceConnection`, `RetryCount`(이동 재시도 횟수, 최대 3) |
...
// 2. 도착 판정: MovementWaypoints 비활성화 OR (사거리 내 + 저속)
...
// 3. 이동 완료 + 사거리 밖 → 재시도/포기
if (!isInRange)
{
    if (pendingData.RetryCount < MaxBuildRetryCount)  // MaxBuildRetryCount = 3
    {
        // 재시도: BuildSiteCenter 직접 목표로 변경 (offset 목적지가 도달 불가 셀에 배치되는 문제 방지)
        // MovementGoal 갱신 + MovementWaypoints 재활성화
    }
    else
    {
        // 포기: PendingBuildServerData 제거 + Intent=Idle
    }
    continue;
}
