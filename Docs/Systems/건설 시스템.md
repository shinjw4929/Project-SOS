...
| BuildArrivalSystem.cs | SimulationSystemGroup (UpdateAfter: MovementArrivalSystem) | PendingBuildServerData 보유 유닛 + MovementWaypoints 비활성화 감지 → ArrivalUtility.GetInteractionArrivalDistance + IsWithinInteractionRangeXZ로 거리 판정 → 사거리 내: 자원 확인/차감 → 건물 생성 → 정리. 사거리 밖: RetryCount < 3이면 목적지 재계산 + 이동 재개, 3회 초과 시 포기(Intent=Idle) |
...
if (!isInRange)
{
    ...
    {
        // 목적지 재계산 (현재 위치 → BuildSiteCenter 방향, offset = StructureRadius + workRange * 0.5f)
