...
| BuildArrivalSystem.cs | SimulationSystemGroup (UpdateAfter: MovementArrivalSystem) | PendingBuildServerData 보유 유닛 도착 감지 (MovementWaypoints 비활성화 OR 사거리 내+저속<0.1f) → arrivalDist = GetGridCompensatedArrivalDistance(StructureRadius, workRange, CellSize) → 사거리 내: 자원 확인/차감 → 건물 생성 → 정리. 사거리 밖: 즉시 포기(Intent=Idle) |
...
| 파일 | 역할 |
| --- | --- |
| ArrivalUtility.cs | 도착 거리 공통 유틸리티 (Burst 호환). GetSafeArrivalRadius(Dead Zone 없는 ArrivalRadius 계산), GetGridCompensatedArrivalDistance(그리드 이동 오차 보정 도착 거리 = targetRadius + workRange + cellSize), IsWithinInteractionRangeXZ(XZ 2D 거리 판정). HandleBuildMoveRequestSystem, BuildArrivalSystem에서 공유 |
...
```csharp
// 1. 거리 판정 (ArrivalUtility 사용, CellSize 마진 포함)
