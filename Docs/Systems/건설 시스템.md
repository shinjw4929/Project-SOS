...
| HandleBuildMoveRequestSystem.cs | SimulationSystemGroup (UpdateAfter: UnifiedTargetingSystem) | BuildMoveRequestRpc 수신 → GhostIdMap으로 Entity 변환, 소유권 검증, MovementGoal.Destination=BuildSiteCenter, BuildApproachRadius 추가(AABB 표면 기준 조기 정지, Value=workRange, HalfW/HalfL=Footprint 절반), UnitIntentState=Build, AggroTarget 초기화, PendingBuildServerData 추가, MovementWaypoints 활성화 |
| BuildArrivalSystem.cs | SimulationSystemGroup (UpdateAfter: MovementArrivalSystem) | PendingBuildServerData 보유 유닛 도착 감지 (MovementWaypoints 비활성화 OR 사거리 내+저속<0.1f) → AABB 표면 거리² 기반 도착 판정 (arrivalDist = workRange + CellSize) → 사거리 내: 자원 확인/차감 → 건물 생성 → PendingBuildServerData + BuildApproachRadius 제거. 사거리 밖: 즉시 포기(Intent=Idle) |
...
| 파일 | 역할 |
| --- | --- |
| ArrivalUtility.cs | 도착 거리 공통 유틸리티 (Burst 호환). GetSafeArrivalRadius(Dead Zone 없는 ArrivalRadius 계산), DistanceSqToAABBSurfaceXZ(AABB 표면까지 XZ 거리², 건설 도착 판정용), GetGridCompensatedArrivalDistance(중심 기반 도착 거리, 채집용), IsWithinInteractionRangeXZ(XZ 2D 거리 판정) |
...
| BuildApproachRadius.cs | 건설 이동 시 FlowField 조기 정지 기준: `Value`(workRange), `Center`(BuildSiteCenter), `HalfW`/`HalfL`(Footprint 절반, 월드 단위). AABB 표면 거리² < Value²이면 웨이포인트 생성 중단. HandleBuildMoveRequestSystem이 추가, BuildArrivalSystem이 제거 |
...
```csharp
// 1. AABB 표면 거리² 기반 판정 (방향 독립적, sqrt 회피)
...
if (!isInRange)
{
    // 포기: PendingBuildServerData + BuildApproachRadius 제거 + Intent=Idle
    continue;
}
