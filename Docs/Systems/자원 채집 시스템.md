...
| 파일 | 그룹 | 역할 |
| --- | --- | --- |
| HandleGatherRequestSystem.cs | SimulationSystemGroup | GatherRequestRpc 수신 → 점유 확인, GatheringTarget 설정, Intent.Gather + Phase 결정, MovementGoal 설정 (ArrivalUtility.CalculateApproachPoint으로 접근점 계산, ArrivalUtility.GetSafeArrivalRadius로 ArrivalRadius 설정) |
| HandleReturnResourceRequestSystem.cs | SimulationSystemGroup | ReturnResourceRequestRpc 수신 → CarriedAmount>0 검증, LastGatheredNode 저장, Phase.MovingToReturn 설정 (ArrivalUtility.CalculateApproachPoint으로 접근점 계산, ArrivalUtility.GetSafeArrivalRadius로 ArrivalRadius 설정) |
| WorkerGatheringSystem.cs | SimulationSystemGroup (UpdateAfter: HandleBuildRequestSystem) | 채집 사이클 5단계 처리. ProcessMovingToGather → ProcessGathering → ProcessMovingToReturn → ProcessUnloading → ProcessWaitingForNode. 내부 헬퍼: TransitionToReturn(반납 전환, ArrivalUtility.CalculateApproachPoint 사용), GetArrivalDistance(ArrivalUtility.GetInteractionArrivalDistance 위임), SetIdleState(Idle 전환) |
...
| 파일 | 역할 |
| --- | --- |
| ArrivalUtility.cs | 도착 거리 공통 유틸리티 (Burst 호환). CalculateApproachPoint(접근점 계산), GetInteractionArrivalDistance(상호작용 도착 거리 = targetRadius + workRange), GetSafeArrivalRadius(Dead Zone 없는 ArrivalRadius), IsWithinInteractionRange/IsWithinInteractionRangeXZ(범위 내 판정). HandleGatherRequestSystem, HandleReturnResourceRequestSystem, WorkerGatheringSystem에서 공유 |
...
참고: Intent.Build 상태에는 이 면제가 적용되지 않는다. 건설 유닛은 다른 유닛에 밀릴 수 있으므로 BuildArrivalSystem에서 재시도 메커니즘(max 3)으로 보상한다.
