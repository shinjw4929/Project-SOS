...
└─ MovementGoal 설정 (노드 표면으로)
       ↓
FlowFieldSystem → FlowFieldSteeringSystem → PredictedMovementSystem (Gather 유닛끼리 Separation 면제)
       ↓
...
| 파일 | 그룹 | 역할 |
| --- | --- | --- |
| HandleGatherRequestSystem.cs | SimulationSystemGroup | GatherRequestRpc 수신 → 점유 확인, GatheringTarget 설정, Intent.Gather + Phase 결정, MovementGoal 설정 (ArrivalUtility.CalculateApproachPoint effectiveRadius 오버로드로 접근점 계산, ArrivalUtility.GetSafeArrivalRadius로 ArrivalRadius 설정) |
| HandleReturnResourceRequestSystem.cs | SimulationSystemGroup | ReturnResourceRequestRpc 수신 → CarriedAmount>0 검증, LastGatheredNode 저장, Phase.MovingToReturn 설정 (ArrivalUtility.CalculateApproachPoint effectiveRadius 오버로드로 접근점 계산, ArrivalUtility.GetSafeArrivalRadius로 ArrivalRadius 설정) |
| WorkerGatheringSystem.cs | SimulationSystemGroup (UpdateAfter: HandleBuildRequestSystem) | 채집 사이클 5단계 처리. ProcessMovingToGather → ProcessGathering → ProcessMovingToReturn → ProcessUnloading → ProcessWaitingForNode. XZ 거리 판정 + CellSize 마진 보정 도착 거리. 내부 헬퍼: TransitionToReturn(반납 전환, effectiveRadius 접근점 + ArrivalRadius 설정), DecideNextAction(자동 복귀, effectiveRadius 접근점 + ArrivalRadius 설정), GetArrivalDistance(ArrivalUtility.GetGridCompensatedArrivalDistance 위임), SetArrivalRadius(Dead Zone 방지), SetIdleState(Idle 전환) |
...
## 공용 유틸리티 (Shared/Utilities/)

| 파일 | 역할 |
| --- | --- |
| ArrivalUtility.cs | 도착 거리 공통 유틸리티 (Burst 호환). CalculateApproachPoint(접근점 계산, effectiveRadius 오버로드 포함), GetGridCompensatedArrivalDistance(그리드 이동 오차 보정 도착 거리 = targetRadius + workRange + cellSize), GetEffectiveRadius(그리드 차단 반경 고려 유효 반지름), GetSafeArrivalRadius(Dead Zone 없는 ArrivalRadius), IsWithinInteractionRangeXZ(XZ 범위 내 판정). HandleGatherRequestSystem, HandleReturnResourceRequestSystem, WorkerGatheringSystem, BuildArrivalSystem에서 공유 |
...
PredictedMovementSystem에서 Intent.Gather 상태인 유닛끼리는 Entity Separation이 면제된다 (`shouldCollide = iAmEnemy || isEnemy || (!iAmGathering && !isGathering)`). 이를 통해 여러 Worker가 같은 자원 노드 근처에 겹쳐서 대기/채집할 수 있다. 적-유닛 간 Separation은 여전히 활성화된다.

참고: Intent.Build 상태에는 이 면제가 적용되지 않는다 (별도 이슈로 추적 중).
