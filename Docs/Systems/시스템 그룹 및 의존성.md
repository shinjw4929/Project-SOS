...
| GamePhaseInitSystem | Server | - | GamePhaseState + ClientServerTickRate(60Hz sim/30Hz net) + GhostDistanceImportance 싱글톤 생성 |
...
| **UnifiedTargetingSystem** | UpdateAfter: SpatialPartitioningGroup, HandleAttackRequestSystem <br> UpdateBefore: FlowFieldSystem | 타겟팅 (공간 분할 기반) |
| GridObstacleResponseSystem | UpdateBefore: FlowFieldSystem | 건물 건설 시 IsPathBlocked 마킹 + 유닛 밀어내기 + 캐시 무효화 |
| GridObstacleCleanupSystem | UpdateAfter: ServerDeathSystem, UpdateBefore: FlowFieldSystem | 건물 파괴 시 IsPathBlocked 해제 + 캐시 무효화 |
| FlowFieldSystem | UpdateAfter: GridObstacleResponseSystem | BFS Flow Field 계산 (LRU 캐시, 8 IJob 병렬) |
| FlowFieldSteeringSystem | UpdateAfter: FlowFieldSystem, UpdateBefore: PredictedMovementSystem | Flow Field 방향 → MovementWaypoints 주입 |
| PredictedMovementSystem | UpdateAfter: FlowFieldSteeringSystem | 이동 + 충돌 회피 |
...
| ~~NavMeshObstacleCleanupSystem~~ | ~~UpdateAfter: ServerDeathSystem~~ | GridObstacleCleanupSystem으로 대체 |
...
| **TokenValidationSystem** | UpdateBefore: GoInGameServerSystem | 토큰 검증 → TokenValidatedTag + RoomSessionInfo 부착 (managed SystemBase) |
| **SlotNotifySystem** | - | 연결 끊김 시 SlotReleased + 30초 하트비트 전송 (managed SystemBase) |
| GoInGameServerSystem | - | 클라이언트 접속 시 Hero 생성 + GhostConnectionPosition + ConnectionViewExtent 부착 (WithAll\<TokenValidatedTag\> 필터 추가) |
| UpdateConnectionPositionSystem | - | CameraPositionRpc → GhostConnectionPosition + ConnectionViewExtent 동기화 |
| **GhostRelevancySystem** | UpdateAfter: UpdateConnectionPositionSystem | Ghost Relevancy AABB 필터링 (적 + 다른 유저 유닛, 자기 유닛 skip, ViewHalfExtent × 1.3/1.15 Hysteresis) |
| MinimapDataBroadcastSystem | - | 적/유닛 위치 MinimapBatchRpc 분산 브로드캐스트 (단일 float3 파이프라인, 매 틱 2배치) |
...
| MinimapDataReceiveSystem | - | MinimapBatchRpc 수신 → MinimapDataState 갱신 (Double buffer 스왑, 적/유닛 수 카운트) |
...
| CommandMarkerPoolInitSystem | - | 커맨드 마커 풀 초기화 (타입당 4개, 1회 실행) |
| CommandMarkerFadeSystem | - | 커맨드 마커 Scale 선형 감소 + 수명 만료 시 Scale=0 (풀링) |
| TeamColorSystem | - | 런타임 초기화(Parent→Team 탐색, ECB) + 팀 색상 틴트 갱신(ScheduleParallel, teamId<=0은 빈 틴트) |
...
[이동] SimulationSystemGroup (Server)
    ...
    → PredictedMovementSystem (MovementMap 사용) → MovementArrivalSystem → BuildArrivalSystem
...
[정리] SimulationSystemGroup (Server)
    ...
    → GridObstacleCleanupSystem + TechStateRecalculateSystem

[후처리] LateSimulationSystemGroup
    GridOccupancyEventSystem

[네트워크 최적화] SimulationSystemGroup (Server)
    ...
    MinimapDataBroadcastSystem → MinimapBatchRpc (적/유닛 위치+teamId, 단일 파이프라인)

[미니맵] SimulationSystemGroup (Client)
    MinimapDataReceiveSystem → MinimapDataState (Double buffer 스왑, 적/유닛 수 카운트)
    MinimapRenderer (MonoBehaviour) → Texture2D 렌더링 (자기 유닛: Ghost 쿼리, 다른 유저 유닛: RPC 데이터)

[커맨드 마커] SimulationSystemGroup (Client)
    CommandMarkerPoolInitSystem → 타입당 4개 풀 초기화 (1회 실행)
    CommandMarkerFadeSystem → Scale 선형 감소 + 수명 만료 시 Scale=0 (풀링, Structural Change 없음)
...
- `GamePhaseInitSystem`에서 `GhostDistanceData` + `GhostImportance` 싱글톤 생성 (TileSize=10)
- 같은 시스템에서 `ClientServerTickRate` 싱글톤 생성 (SimulationTickRate=60, NetworkTickRate=30)
- `DefaultSnapshotPacketSize`: 기본 MTU(~1400B) 사용 (AABB Relevancy로 relevant Ghost ~200-400 제한)
- `GoInGameServerSystem`에서 Connection에 `GhostConnectionPosition` + `ConnectionViewExtent` 부착
- `CameraSystem`에서 ~20Hz로 `CameraPositionRpc` 전송 (Position: Y=0 XZ좌표, ViewHalfExtent: 뷰포트 반크기)
- `UpdateConnectionPositionSystem`에서 CameraPositionRpc 수신 → GhostConnectionPosition + ConnectionViewExtent 반영
- Netcode 내장 `GhostDistancePartitioningSystem`이 자동으로 Ghost에 타일 인덱스 부여
- 카메라 근처 Ghost는 높은 빈도로 스냅샷 전송, 먼 Ghost는 빈도 감소
- `GhostRelevancySystem`에서 **AABB 판정** (ViewHalfExtent × 1.3 Outer / × 1.15 Inner) → 뷰포트 밖 적/다른 유저 유닛 Ghost SetIsIrrelevant → 전송 완전 차단 (자기 유닛은 GhostOwner 기반 skip)
- 해상도/비율 자동 대응: 클라이언트가 뷰포트 4코너를 y=0 투영하여 반크기 계산

### 미니맵 RPC 브로드캐스트 패턴
- `MinimapDataBroadcastSystem` (Server): 적/유닛 위치를 단일 float3(xz+teamId)로 수집, 32개씩 MinimapBatchRpc로 분할, 매 틱 2배치씩 분산 전송
- `MinimapDataReceiveSystem` (Client): RPC 수신 → MinimapDataState Double buffer 스왑 + 적/유닛 수 카운트
- `MinimapRenderer` (MonoBehaviour): 적(teamId=-1)=RPC 빨간색, 다른 유저 유닛=RPC(teamId로 팀 색상, 자기 teamId skip), 자기 유닛/히어로=Ghost 쿼리(GhostOwnerIsLocal), 건물=Ghost 쿼리
- 대역폭: (적+유닛) 통합 전송, 2600엔티티 기준 ~23KB/s per connection
