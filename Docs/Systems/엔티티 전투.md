...
HeroDeathDetectionSystem → Hero 사망 감지, UserAliveState 업데이트, HeroDeathRpc/GameOverRpc 전송
               ↓
ServerDeathSystem → DeathDetectionJob: Dying+DeathTimer 부착 → DeathTimerJob: 타이머 만료 시 파괴 + 인구수 반환
               ↓
[Client] EntityTiltSystem → Dying 상태 수신 → 전방 기울임 연출 (PostTransformMatrix)
               ↓
...
| 파일 | 역할 |
| --- | --- |
| CombatStats.cs | 공격력(AttackPower), 공격속도(AttackSpeed), 사거리(AttackRange), 투사체속도(ProjectileSpeed) |
...
| EnemyState.cs | 적 행동 상태(EnemyContext enum, [GhostField]) + 임시 차단 타겟(AbandonedTarget, AbandonedExpireTime). CurrentState만 클라이언트 복제 |
...
| DeathTimer.cs | 사망 연출 타이머. Dying 진입 시 부착, RemainingTime 만료 시 엔티티 파괴. DeathDuration은 GameSettings |
...
그룹: SimulationSystemGroup (UpdateAfter: SpatialPartitioningGroup, HandleAttackRequestSystem, UpdateBefore: FlowFieldSystem)
...
- **IsPathPartial 자동 재평가**: dest를 항상 targetPos로 갱신. FlowFieldSystem Apply에서 `IsPathPartial = (IsDestAdjusted == 1)` 매 프레임 자동 판정. 타겟이 벽 밖으로 이동하면 즉시 `IsPathPartial=false` → 정상 추격 재개.
- **대체 타겟 탐색 (IsPathPartial + Chasing)**: 4프레임 1회(`TimeSliceDivisor`) 프레임 분산으로 현재 타겟 제외 대체 타겟 탐색. 대체 발견 시 즉시 전환, 없으면 기존 타겟 유지.
- **Chasing stuck 감지 → AbandonedTarget 차단**: `WanderUtility.CheckStuck`으로 3초 간격 이동 거리 체크. 벽 앞에서 2m 미만 이동 시 현재 타겟을 `EnemyState.AbandonedTarget`에 기록 (30초 `GameSettings.TargetAbandonDuration` 임시 차단) → 타겟 탐색/전파에서 해당 타겟 필터링 → Wandering 전환.
- **TargetPropagationJob AbandonedTarget 필터**: 전파 대상 타겟이 AbandonedTarget이고 만료 전이면 전파 차단.
- **건물 파괴 시 즉시 경로 갱신**: GridObstacleCleanupSystem에서 파괴된 건물 12m 반경 내 Partial Path 엔티티의 IsPathDirty를 즉시 true로 설정.
- **배회 stuck 감지 → Dormant 전환**: `WanderUtility.CheckStuck`으로 판정. 3초(`WanderUtility.StuckCheckInterval`)마다 이동 거리 체크 (`MovementGoal.LastPositionCheck`, `LastPositionCheckTime`). 2m(`WanderUtility.StuckThreshold`) 미만 이동 시 stuck → `EnemyContext.Dormant` 전환 (이동/pathfinding 완전 중단, MovementWaypoints 비활성화, IsPathDirty=false). 5~8초 랜덤 타이머(`MovementGoal.DormantWakeTime`) 후 `Idle`로 깨어남. 피격(AggroLock 활성화) 시 즉시 깨어남. 벽 파괴 시 12m 내 Dormant 적 즉시 깨움(GridObstacleCleanupSystem).
...
3. **벽 파괴**: `GridObstacleCleanupSystem`에서 12m 내 Dormant 적 즉시 `Idle`
...
6. `MovementWaypoints` 활성화 → FlowFieldSystem에서 경로 계산
7. FlowFieldSteeringSystem → PredictedMovementSystem에서 이동 실행
...
그룹: SimulationSystemGroup
역할: 2단계 사망 처리. DeathDetectionJob: Health ≤ 0 감지 → UnitActionState/EnemyState를 Dying으로 설정 + DeathTimer 부착 + 인구수 반환 (Ghost 스냅샷에 Dying 포함). DeathTimerJob: DeathTimer.RemainingTime 카운트다운 → 만료 시 ECB.DestroyEntity. 건물 등 비유닛/비적 엔티티는 즉시 파괴. DeathDuration은 GameSettings에서 조정 가능.
...
그룹: SimulationSystemGroup
역할: Health ≤ 0인 비유닛/비적 엔티티(건물 등)에 DisableRendering 추가. 유닛/적은 Dying 기울임 연출 후 서버 파괴 시 Ghost 제거로 사라지므로 제외 (WithNone<EnemyState, UnitActionState>).
---
...
| MovementMath.cs | 이동 계산 유틸리티. ShouldRetryPartialPath(Partial Path 재시도 판정, 현재 EnemyTargetJob에서 미사용 — dest 항상 갱신으로 대체) |
...
UnifiedTargetingSystem (SpatialMaps.TargetingMap 사용)
    ├─ EnemyTargetJob (적→아군 타겟팅, AggroLock 체크, 대체 타겟 탐색, AbandonedTarget 필터)
    ├─ TargetPropagationJob (타겟 없는 적 → 주변 적 타겟 복사, AbandonedTarget 필터)
