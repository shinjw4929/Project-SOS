...
HeroDeathDetectionSystem → Hero 사망 감지, UserAliveState 업데이트, HeroDeathRpc/GameOverRpc 전송
               ↓
ServerDeathSystem → 2단계: Dying 상태 설정 → 다음 프레임 삭제 + 인구수 반환
               ↓
...
그룹: SimulationSystemGroup
역할: 2단계 사망 처리. 1프레임째: Health ≤ 0 감지 → UnitActionState/EnemyState를 Dying으로 설정 + 인구수 반환 (Ghost 스냅샷에 Dying 포함). 2프레임째: Dying 상태 확인 → ECB.DestroyEntity. 건물 등 상태 컴포넌트 없는 엔티티는 즉시 파괴
...
HeroDeathDetectionSystem (UpdateBefore: ServerDeathSystem)
    ...
    → 모든 유저 사망 시 GameOverRpc 브로드캐스트
    ↓
ServerDeathSystem → 2단계: Dying 설정 → 다음 프레임 삭제 + 인구수 반환
