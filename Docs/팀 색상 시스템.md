...
│   └─ ECB로 TeamColorTarget + URPMaterialPropertyBaseColor 부착
│
└─ Phase 2: 색상 갱신 (IJobEntity, ScheduleParallel)
    ├─ TeamColorTarget.RootEntity → Team.teamId 조회
    └─ TeamColorPalette.GetTeamColor(teamId) → URPMaterialPropertyBaseColor.Value 설정

[Runtime - Client] StructurePreviewController (건설 프리뷰)
└─ 프리뷰 생성 시 로컬 유저 teamId 조회 → MeshRenderer.material.SetColor 적용
...
3. **빈 틴트 패턴**: teamId <= 0 (적, 씬 배치)은 `float4(1,1,1,1)` 반환 → 원본 머티리얼 색상 유지. 모든 경로에서 `TeamColorPalette.GetTeamColor()` 호출 (분기 없음)
4. **클라이언트 간 동일 색상**: `GhostOwnerIsLocal` 미사용. 모든 클라이언트가 `teamId` 기준으로 동일 색상 확인
...
| 파일 | 그룹 | 역할 |
| --- | --- | --- |
| TeamColorSystem.cs | SimulationSystemGroup (ClientSimulation) | Phase 1: 새 메시 엔티티에 TeamColorTarget + URPMaterialPropertyBaseColor 부착 (ECB). Phase 2: TeamColorJob(IJobEntity, ScheduleParallel)으로 팀 색상 갱신 |
...
| 파일 | 역할 |
| --- | --- |
| StructurePreviewController.cs | 건설 프리뷰 생성 시 `ApplyTeamColorTint()` 호출. 로컬 유저 HeroTag + GhostOwnerIsLocal + Team 쿼리 → teamId 조회 → `MeshRenderer.material.SetColor("_BaseColor", tint)` |
...
ECB로 컴포넌트 부착
       ...
       └─ URPMaterialPropertyBaseColor { (1,1,1,1) } (RootEntity != Entity.Null일 때만)
```
...
3. ECB로 `TeamColorTarget` + `URPMaterialPropertyBaseColor` 부착
