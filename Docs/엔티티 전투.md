...
- **Partial Path 재시도**: `MovementGoal.IsPathPartial`이면 120프레임(~2초@60fps)마다 경로 재계산. 목적지 변경 시 IsPathPartial 리셋. 벽 파괴 등 장애물 변화에 대응.
...
4. `MovementWaypoints` 활성화 → PathfindingSystem에서 경로 계산
5. PathFollowSystem에서 이동 실행
6. 목적지 도착 또는 타겟 발견 시 다음 상태로 전환
