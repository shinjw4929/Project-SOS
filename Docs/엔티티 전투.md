...
- **배회 stuck 감지**: `WanderUtility.CheckStuck`으로 판정. 3초(`WanderUtility.StuckCheckInterval`)마다 이동 거리 체크 (`MovementGoal.LastPositionCheck`, `LastPositionCheckTime`). 2m(`WanderUtility.StuckThreshold`) 미만 이동 시 stuck으로 판정하여 `WanderUtility.GenerateWanderDestination`으로 재배회.
...
**stuck 감지** (WanderUtility.CheckStuck):
```csharp
// 3초간 2m 미만 이동 시 stuck 판정
...
| 파일 | 역할 |
| --- | --- |
| CombatUtility.cs | 전투 공통 유틸리티. CalculateEffectiveDistance(유효 거리), TickCooldown(쿨다운 감소), RotateTowardTarget(타겟 회전), ResetCooldown(쿨다운 리셋), SpawnVisualProjectile(시각 투사체 생성) |
| DamageUtility.cs | 데미지 계산: finalDamage = baseDamage * (1 - defense). Defense 0.3이면 30% 감소 |
| WanderUtility.cs | 적 배회 유틸리티. CheckStuck(stuck 감지), GenerateWanderDestination(랜덤 배회 목적지 생성) |
...
```csharp
// 직선거리 - 타겟 반지름(ObstacleRadius) → CombatUtility.CalculateEffectiveDistance
