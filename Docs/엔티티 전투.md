...
└──────────────┼──────────────┘
        ...
        AggroReactionSystem → 피격 시 어그로 전환
               ↓
        DamageApplySystem → Health 적용 + 버퍼 클리어
               ↓
[SimulationSystemGroup]
...
역할: 원거리 공격 처리.
- ProcessRangedAttack: RangedUnitTag 유닛 처리 (Trooper, Sniper)
- ProcessEnemyRangedAttack: RangedEnemyTag 적 처리 (EnemyFlying)
- 필중 시스템: 거리 판정 후 즉시 DamageEvent 추가 (Attacker 포함) + 시각 투사체 생성(VisualOnlyTag)
...
그룹: FixedStepSimulationSystemGroup (UpdateAfter: MeleeAttackSystem)
역할: DamageEvent 버퍼 합산 → Health에 적용 → 버퍼 클리어.
...
DamageApplySystem (데미지 적용 + 버퍼 클리어)
