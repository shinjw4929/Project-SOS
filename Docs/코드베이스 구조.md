...
└── Systems/
    ...
    ├── FireProjectileServerSystem.cs        # 투사체 발사 (OrderLast)
    ├── Gathering/
    ...
    │   ├── PathfindingSystem.cs             # Pathfinding 시스템 (UpdateAfter: NavMeshObstacleSpawnSystem)
    ...
    ├── Spatial/
    │   ├── SpatialMapBuildSystem.cs         # 공간 분할 맵 빌드 (TargetingMap + MovementMap, OrderFirst)
    │   └── SpatialMapDisposeSystem.cs       # 공간 분할 맵 해제 (LateSimulation, CompleteDependency)
    ├── TechStateRecalculateSystem.cs        # 기술 상태 재계산 시스템 (UpdateAfter: ServerDeathSystem)
    └── Wave/
        ├── EnemyDeathCountSystem.cs         # 적 처치 수 카운팅 (UpdateBefore: ServerDeathSystem)
