...
├── GoInGameServerSystem.cs                  # 클라이언트 접속 시 Hero 생성 + UserAliveState + GhostConnectionPosition 추가
└── Systems/
    ├── Combat/
    ...
    ├── Initialize/
    │   └── GamePhaseInitSystem.cs           # GamePhaseState + GhostDistanceImportance 싱글톤 초기화
    ...
    ├── UpdateConnectionPositionSystem.cs   # Hero→Connection GhostConnectionPosition 동기화 (GhostDistanceImportance용)
    ├── Spatial/
