...
| BuildArrivalSystem.cs | SimulationSystemGroup (UpdateAfter: MovementArrivalSystem) | PendingBuildServerData 보유 유닛 + MovementWaypoints 비활성화 감지 → ArrivalUtility.GetInteractionArrivalDistance + IsWithinInteractionRangeXZ로 거리 판정 → 자원 확인/차감 → 건물 생성 → PendingBuildServerData 제거, ArrivalRadius 초기화, Intent=Idle |
