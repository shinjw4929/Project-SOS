...
| HandleBuildMoveRequestSystem.cs | SimulationSystemGroup (UpdateAfter: UnifiedTargetingSystem) | BuildMoveRequestRpc 수신 → GhostIdMap으로 Entity 변환, 소유권 검증, MovementGoal 설정, UnitIntentState=Build, AggroTarget 초기화, PendingBuildServerData 추가, MovementWaypoints 활성화 |
| BuildArrivalSystem.cs | SimulationSystemGroup (UpdateAfter: MovementArrivalSystem) | PendingBuildServerData 보유 유닛 + MovementWaypoints 비활성화 감지 → 거리 검증(arrivalThreshold = unitRadius + 1.0f) → 자원 확인/차감 → 건물 생성 → PendingBuildServerData 제거, Intent=Idle |
