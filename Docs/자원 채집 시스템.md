...
| ResourceNodeCleanupSystem.cs | SimulationSystemGroup (OrderLast=true) | 점유 상태 유효성 검사. Worker 사망/삭제/타겟 변경 시 점유 해제 |
...
| WorkerCarriedResourceLinked.cs | Worker에 CarriedResource가 이미 생성됨을 표시 (중복 생성 방지) |
...
| 다른 노드로 타겟 변경 | HandleGatherRequestSystem |
...
WorkerGatheringSystem → 채집 사이클 5단계 처리

[SimulationSystemGroup - Server, OrderLast=true]
ResourceNodeCleanupSystem → 점유 유효성 검사
