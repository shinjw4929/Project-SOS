...
| **GhostRelevancySystem** | UpdateAfter: UpdateConnectionPositionSystem | Ghost Relevancy AABB 필터링 (ViewHalfExtent × 1.3/1.15 Hysteresis) |
| MinimapDataBroadcastSystem | - | 적 위치 MinimapBatchRpc 분산 브로드캐스트 (매 틱 2배치) |
...
| MinimapDataReceiveSystem | - | MinimapBatchRpc 수신 → MinimapDataState 갱신 (Double buffer 스왑) |
...
[네트워크 최적화] SimulationSystemGroup (Server)
    UpdateConnectionPositionSystem (CameraPositionRpc 수신) → GhostRelevancySystem (뷰포트 AABB × 1.3 밖 적 Ghost 전송 제외)
    MinimapDataBroadcastSystem → MinimapBatchRpc (적 위치 RPC 배치 전송)
...
- `GhostRelevancySystem`에서 **AABB 판정** (ViewHalfExtent × 1.3 Outer / × 1.15 Inner) → 뷰포트 밖 적 Ghost SetIsIrrelevant → 전송 완전 차단
- 해상도/비율 자동 대응: 클라이언트가 뷰포트 4코너를 y=0 투영하여 반크기 계산

### 미니맵 RPC 브로드캐스트 패턴
- `MinimapDataBroadcastSystem` (Server): 전체 적 위치를 수집, 32개씩 `MinimapBatchRpc`로 분할, 매 틱 2배치씩 분산 전송
- `MinimapDataReceiveSystem` (Client): FrameId 기반 프레임 구분, PendingPositions에 수신 → 완료 시 EnemyPositions와 스왑
- `MinimapRenderer` (MonoBehaviour): MinimapDataState에서 적 위치 읽기, Ghost 쿼리로 아군/건물/히어로 표시
- 대역폭: 2400적 기준 ~20KB/s per connection (265B/RPC)
