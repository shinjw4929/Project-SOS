...
| GamePhaseInitSystem | Server | - | GamePhaseState 싱글톤 생성 |
...
| PathfindingSystem | UpdateAfter: NavMeshObstacleSpawnSystem | NavMesh 경로 계산 |
| PathFollowSystem | UpdateAfter: PathfindingSystem | 웨이포인트 버퍼 관리 |
...
| EnemyDeathCountSystem | UpdateBefore: ServerDeathSystem | 적 처치 카운트 |
...
| HeroDamageOnContactServerSystem | - | Hero 접촉 데미지 |
| WaveManagerSystem | - | Wave 전환 조건 체크 |
| EnemySpawnerSystem | UpdateAfter: WaveManagerSystem | 적 스폰 |
| GoInGameServerSystem | - | 클라이언트 접속 시 Hero 생성 |
| FireProjectileServerSystem | OrderLast=true | RPC 기반 투사체 발사 |
...
| SpatialMapDisposeSystem | Server | 공간 분할 맵 해제 (CompleteDependency 후) |
...
| HeroHpTextPresentationSystem | Hero HP 텍스트 UI |
...
[이동] SimulationSystemGroup (Server)
    PathfindingSystem → PathFollowSystem → PredictedMovementSystem (MovementMap 사용)
...
[정리] SimulationSystemGroup (Server)
    EnemyDeathCountSystem → HeroDeathDetectionSystem → ServerDeathSystem
...
5. `DamageApplySystem`: DamageEvent 합산 → Health 적용 + 버퍼 클리어
